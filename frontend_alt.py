...
class AnalyzerApp(tk.Tk):
    def __init__(self) -> None:
        super().__init__()
        self.title("Machine/Operator Analyzer - Alt")
    ...
    def _add_header_images(self) -> None:
        ...
        def _load_image(path: Path) -> ImageTk.PhotoImage | None:
            ...
            with Image.open(path) as img:
                resized = img.resize((240, 180), Image.Resampling.LANCZOS)
                return ImageTk.PhotoImage(resized)

        base_path = Path(__file__).resolve().parent
        images_path = base_path / "images"

        self._left_image = _load_image(images_path / "rev_gru.jpg")
        self._right_image = _load_image(images_path / "rev_meme.webp")

        left_label = tk.Label(image_frame)
        left_label.pack(side=tk.LEFT, anchor="nw")
        if self._left_image is not None:
            left_label.configure(image=self._left_image)
        else:
            left_label.configure(text="rev_gru.jpg missing", font=("Segoe UI", 9))

        right_label = tk.Label(image_frame)
        right_label.pack(side=tk.RIGHT, anchor="ne")
        if self._right_image is not None:
            right_label.configure(image=self._right_image)
        else:
            right_label.configure(text="rev_meme.webp missing", font=("Segoe UI", 9))
    ...
    def _populate_machine_columns(self, result) -> None:
        ...
        for machine_name, report in result.machines.items():
            ...
            column.configure(width=280)
            column.pack_propagate(False)

            title = tk.Label(column, text=machine_name, font=("Segoe UI", 13, "bold"))
            ...
            tk.Label(
                column, text="Machine activations without operator", font=("Segoe UI", 10, "bold")
            ).pack(anchor="w", padx=8, pady=(8, 2))
            machine_list = tk.Listbox(column, height=8, width=36)
            ...
            tk.Label(
                column, text="Operator presence without machine", font=("Segoe UI", 10, "bold")
            ).pack(anchor="w", padx=8, pady=(8, 2))
            operator_list = tk.Listbox(column, height=8, width=36)
